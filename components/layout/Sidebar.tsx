...
export default function Sidebar(){const pathname=usePathname();return <aside style={{width:220,minHeight:"100vh",background:"#fff",borderRight:"1px solid #e8edf5",padding:"18px 14px"}}><div style={{fontWeight:800,fontSize:20,marginBottom:24}}>Toy & Stationery POS</div><nav style={{display:"grid",gap:8}}>{links.map(({href,label,icon:Icon},index)=>{const active=pathname===href||(href!=="/dashboard"&&pathname.startsWith(href));return <Link className="sidebar-menu-link" key={`${href}-${index}`} href={href} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 12px",borderRadius:10,background:active?"#edf4ff":"transparent",color:active?"#1769e0":"#30405f",fontWeight:active?700:500}}><Icon size={18}/>{label}</Link>})}</nav></aside>}
