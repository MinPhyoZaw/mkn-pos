...
export default function SalesHistoryPage() {
	...
	return <AppShell><div className="sales-history-page">
		<header className="sales-history-header"><div><h1>Sales History</h1><p>Review completed and cancelled transactions.</p></div></header>
		{notice && <div className="history-notice">{notice}</div>}{error && <div className="history-error">{error}</div>}
		<section className="history-summary"><Summary label="Today's Sales" value={money(todaySales)} tone="blue" /><Summary label="This Month Sales" value={money(monthSales)} tone="green" /><Summary label="This Month Gross Profit" value={money(monthProfit)} tone="violet" /><Summary label="Total Transactions" value={String(sales.length)} tone="orange" /></section>
		<section className="history-panel"><div className="history-filters"><input placeholder="Search Sale #..." value={search} onChange={(event) => setSearch(event.target.value)} /><select value={dateRange} onChange={(event) => setDateRange(event.target.value as DateRange)}><option value="ALL_TIME">All Time</option><option value="TODAY">Today</option><option value="LAST_7_DAYS">Last 7 Days</option><option value="THIS_MONTH">This Month</option></select><select value={source} onChange={(event) => setSource(event.target.value as "ALL" | SaleSource)}><option value="ALL">Source: All</option><option value="POS">POS</option><option value="ORDER">Order</option></select><select value={status} onChange={(event) => setStatus(event.target.value as "ALL" | SaleStatus)}><option value="ALL">Status: All</option><option value="COMPLETED">Completed</option><option value="CANCELLED">Cancelled</option></select></div>
			<div className="history-table-wrap"><table className="history-table"><thead><tr><th>Sale ID</th><th>Date &amp; Time</th><th>Items</th><th>Total Quantity</th><th>Total Amount</th><th>Gross Profit</th><th>Source</th><th>Status</th><th>Actions</th></tr></thead><tbody>
