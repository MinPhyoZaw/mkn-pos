...
export default function OrdersPage() {
  ...
  return <AppShell><div className="orders-page">
    <header className="orders-header"><div><h1>အော်ဒါစာမျက်နှာ</h1><p>Manage customer orders and payment status</p></div><button className="primary" onClick={openCreate}>+ New Order</button></header>
    {notice && <div className="notice">{notice}</div>}{error && !formOpen && <div className="error">{error}</div>}
    <section className="summary-grid">{[["စုစု‌ပေါင်းအော်ဒါများ", orders.length], ["အော်ဒါ အတည်ပြုရန်", orders.filter(o=>o.status==="PENDING").length], ["ငွေပေးချေရန်ကျန်သော အော်ဒါ", orders.filter(o=>o.paymentStatus==="UNPAID").length], ["အော်ဒါအောင်မြင်မှု", orders.filter(o=>o.status==="COMPLETED").length]].map(([name,value])=><div className="summary" key={name}><span>{name}</span><strong>{value}</strong></div>)}</section>
    ...
    <div className="table-card"><table><thead><tr><th>Order</th><th>ဈေး၀ယ်သူ</th><th>ဖုန်းနံပါတ်</th><th>ရက်စွဲ</th><th>စုစုပေါင်း</th><th>ငွေပေးချေမှု့</th><th>အော်ဒါအခြေနေ</th><th>လုပ်ဆောင်ချက်</th></tr></thead><tbody>
