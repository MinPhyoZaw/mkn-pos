...
export default function ReportsPage() {
	...
	return <AppShell><div className="reports-page">
		...
		{!loading && report && <>
			<section className="reports-summary"><ReportCard label="စုစုပေါင်းရောင်းရငွေ" value={money(report.summary.totalSales)} tone="green" /><ReportCard label="အသားတင်အမြတ်" value={money(report.summary.grossProfit)} tone="blue" /><ReportCard label="ရောင်းရသောအကြိမ်ရေအတွက်" value={String(report.summary.transactions)} tone="violet" /><ReportCard label="ရောင်းထွက်သော ကုန်ပစ္စည်းအရေအတွက်" value={String(report.summary.itemsSold)} tone="orange" /></section>
			...
			<div className="reports-grid"><section className="report-panel"><div className="report-heading"><div><h2>အရောင်းရဆုံး ကုန်ပစွည်းများ</h2><p>Ranked by quantity sold.</p></div></div>{!report.topProducts.length ? <Empty text="No product sales in this period." /> : <ReportTable headers={["ကုန်ပစ္စည်းအမည်", "ရောင်းရသောအရေအတွက်", "စုစုပေါင်းတန်ဖိုး", "အမြတ်"]} rows={report.topProducts.map((item) => [<strong key="name">{item.productName}</strong>, item.quantitySold, money(item.revenue), money(item.grossProfit)])} />}</section>
